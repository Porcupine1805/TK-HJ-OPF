...
public final class TKHJOPFMiner {
    ...
    public MiningResult mine(double[] t,double k,int topK,int maxLen){
        return mine(t,k,topK,2,maxLen,true,true,true);
    }

    public MiningResult mine(double[] t,double k,int topK,int minLen,int maxLen){
        return mine(t,k,topK,minLen,maxLen,true,true,true);
    }

    public MiningResult mine(double[] t,double k,int topK,int minLen,int maxLen,boolean usePDUB,boolean useDUB){
        return mine(t,k,topK,minLen,maxLen,usePDUB,useDUB,true);
    }

    /**
     * @param useProfile if true, materialize depth-UB profiles once per pattern per generation
     *                   and evaluate DUB/PDUB in O(D); if false, recompute from occurrence
     *                   breakpoints at every call (naive ablation).
     */
    public MiningResult mine(double[] t,double k,int topK,int minLen,int maxLen,boolean usePDUB,boolean useDUB,boolean useProfile){
        ...
        profileAll(cur,t.length,k,maxLen,useProfile);
        if(useDUB) cur=branchFilter(cur,t.length,k,w,maxLen,heap,topK,met,useProfile);met.sampleHeap();
        for(int m=2;m<maxLen&&!cur.isEmpty();m++){
            ...
            for(PatternData p:cur){
                ...
                for(PatternData q:bucket){
                    ...
                    // Algorithm: evaluate PDUB only when the heap is full (theta>0).
                    if(usePDUB && heap.size()>=topK){
                        double pdub=Bounds.pairDescendantUpperBound(p,q,t.length,k,w,childDepthBudget,useProfile);
                    ...
                    for(PatternData r:Fusion.fuse(p,q,t,w,met)){
                        ...
                        if(remain>0){
                            if(!useDUB || heap.size()<topK || Bounds.descendantUpperBound(r,t.length,k,w,remain,useProfile)+PRUNE_EPS>=now) next.put(r.pattern,r);
    ...
    private static void profileAll(List<PatternData> cur, int n, double k, int maxLen, boolean useProfile){
        if(!useProfile) return;
        for(PatternData p:cur) p.ensureUbProfile(n,k,Math.max(0,maxLen-p.pattern.length()));
    }

    private static List<PatternData> branchFilter(List<PatternData>x,int n,double k,double[]w,int maxLen,PriorityQueue<ScoredPattern>h,int K,Metrics m,boolean useProfile){
        ArrayList<PatternData>o=new ArrayList<>();double theta=theta(h,K);
        for(PatternData p:x){int d=maxLen-p.pattern.length();if(d<=0||h.size()<K||Bounds.descendantUpperBound(p,n,k,w,d,useProfile)+PRUNE_EPS>=theta)o.add(p);else m.branchBoundPrunes++;}
        return o;
    }
