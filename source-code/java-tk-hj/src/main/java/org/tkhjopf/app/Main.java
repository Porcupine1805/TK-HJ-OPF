...
public final class Main{
 public static void main(String[]args)throws Exception{
  ...
  switch(mode){case"baseline"->r=new BaselineOPFMiner().mine(t,k,Double.parseDouble(a.getOrDefault("minSup","1.5")),max);case"hj"->r=new HJOPFMiner().mine(t,k,Double.parseDouble(a.getOrDefault("minSup","1.5")),max);case"hjtopk"->r=new ExhaustiveHJTopKMiner().mine(t,k,Integer.parseInt(a.getOrDefault("topK","10")),minLen,max);case"tk"->r=new TKHJOPFMiner().mine(t,k,Integer.parseInt(a.getOrDefault("topK","10")),minLen,max,true,true);case"tk-no-pdub"->r=new TKHJOPFMiner().mine(t,k,Integer.parseInt(a.getOrDefault("topK","10")),minLen,max,false,true);case"tk-no-dub"->r=new TKHJOPFMiner().mine(t,k,Integer.parseInt(a.getOrDefault("topK","10")),minLen,max,true,false);case"tk-no-bounds"->r=new TKHJOPFMiner().mine(t,k,Integer.parseInt(a.getOrDefault("topK","10")),minLen,max,false,false);default->throw new IllegalArgumentException("mode=baseline|hj|hjtopk|tk|tk-no-pdub|tk-no-dub|tk-no-bounds");}
